...
folder_of_data = '/mnt/beegfs/bianca.fusinato/monan/MASTERS_RESULTS/rainfall_analysis/rainfall_sliced_data/'
#where_save_figure = '/mnt/beegfs/bianca.fusinato/monan/MASTERS_RESULTS/rainfall_analysis/correlation_spatial/'
where_save_figure = '/mnt/beegfs/bianca.fusinato/monan/MASTERS_RESULTS/rainfall_analysis/Appendix_E/'
...
experiments = data_information = ['ERA5','GSMaP', 'CP-ON', 'CP-OFF', 'CP-1H', 'CP-3H', 'CP-D025', 'CP-D050', 'CP-15km', 'CP-60km', 'CP-GFS', 'CP-29', 'CP-02T12', 'CP-1HD050']
...
experiments_selected = ['ERA5', 'CP-OFF', 'CP-ON', 'CP-D050', 'CP-29']

# Letras para os subplots
letters = ['(a)', '(b)', '(c)']

# Criar painel: 5 linhas (experimentos) x 3 colunas (dias)
fig, axes = plt.subplots(nrows=5, ncols=3, figsize=(18, 17), subplot_kw={'projection': ccrs.PlateCarree()})
...
for row_idx, experiment_file in enumerate(experiments_selected):
    for col_idx, (day_name, (initial_day, final_day)) in enumerate(days_selected.items()):
        ...
        if col_idx == 0:
            ax.text(-0.15, 0.5, experiment_file, va='center', ha='right', rotation=90,
