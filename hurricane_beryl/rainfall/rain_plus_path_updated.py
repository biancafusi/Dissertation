...
from namelist_for_rain_pathing import *
...
for label, color, input_file_data in data_information:
    ...
    if label == 'ERA5':
        ...
        prec_acc = model_data.rainc.sel(time=final_day) + model_data.rainnc.sel(time=final_day)
...
# plt.savefig("ALL_tracks_for_presentation.png", dpi=300, bbox_inches='tight', pad_inches=0)
# plt.close()
# #exit()
...
groups = {
    "Cold Pool Effects": ['NOAA', 'CP-ON', 'CP-OFF', 'ERA5'],
    "Cold Pool Lifetime": ['NOAA', 'CP-ON', 'CP-1H', 'CP-3H'],
    "Maximum Downdraft Height": ['NOAA', 'CP-ON', 'CP-D025', 'CP-D050'],
    "Resolution Experiment": ['NOAA', 'CP-ON', 'CP-15km', 'CP-60km'],
    "Initial Condition": ['NOAA', 'CP-ON', 'CP-29', 'CP-02T12'],
    "Type of Initial Condition": ['NOAA', 'CP-ON', 'CP-GFS'],
    "Best Configuration Test": ['NOAA', 'CP-ON', 'CP-1HD050']
}
...
for group_name, exp_list in groups.items():
    ...
    plt.savefig('/mnt/beegfs/bianca.fusinato/monan/MASTERS_RESULTS/accumulated/Appendix_D/'+f"{group_name.replace(' ', '_')}_tracks.png", dpi=300, bbox_inches='tight', pad_inches=0)
    plt.close()
...
#     plt.legend()
#     plt.title(f"Track of {label} + best track (NOAA)")
#     plt.savefig(f"/mnt/beegfs/bianca.fusinato/monan/MASTERS_RESULTS/accumulated/Appendix_B/{label}_track_NOAA.png", dpi=300, bbox_inches='tight', pad_inches=0)
#     plt.close()
