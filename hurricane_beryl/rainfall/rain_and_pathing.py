...
#         dataset_prec = xr.open_dataset(input_file_data).rename({'valid_time': 'time', 'latitude': 'lat', 'longitude': 'lon'})
#         dataset_dry = xr.open_dataset(input_file_data).rename({'valid_time': 'time', 'latitude': 'lat', 'longitude': 'lon', 'msl': 'mslp'})
...
#         slice_prec = dataset_prec.tp.sel(lat=slice(latN, latS), lon=slice(lonW, lonE)) * 1000 * 24# to mm/h
#         prec_acc = slice_prec.sum(dim='time')/num_days
...
#     (MSLP_data, lat_data, lon_data, 's', label_data, 'black'),
# ]
