...
%o \LaTeX\ alinha o texto automaticamente.
	...
	com a palavra fonte. Adaptada de \citeonline{mauri:2003}.}	% fonte consultada (Quando for produção do próprio autor não incluir a fonte.)
%o \LaTeX\ alinha o texto automaticamente.
\end{figure}
...
\begin{table}[!ht]%[htbp] %opções de colocação da tabela no texto
  ...
  \caption{Exemplo de tabela, com fonte.}
    ...
    \end{tabular}
    \end{center}
 \FONTE{Coloque a fonte de referência aqui, se houver.} 	% fonte consultada (Quando for produção do próprio autor não incluir a fonte.)
\end{table}
...
%   \label{\fullpaperid:table:1}% you must prefix your labels (here table:1) with the string \fullpaperid: (this will be important when combining all the full papers for the final book)
    ...
    \FONTE{Coloque a fonte longa longa longa longa longa longa longa longa longa longa aqui, se houver.} 	% fonte consultada (Quando for produção do próprio autor não incluir a fonte.)
\end{table}
...
\vspace{-8mm}
\begin{center}
	Fonte: Referência a fonte da tabela. 	% fonte consultada (Quando for produção do próprio autor não incluir a fonte.)
\end{center}
