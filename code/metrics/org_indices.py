	...
	if ncnv>15:
		...
		for k in range(len(centroids_updraft)):
			...
			if periodic_BCs:
				...
				#print(ncnv)
				#print(centroids_updraft)
				#if ncnv<=2:
				#	print('this case is out')
				#	break
				#else:
				dist,ii=tree.query(centroids_updraft[k,:], ncnv-1)
			...
			#print(f'NNdist vindo do org_indices: {type(NNdist)}')
			#print(f'dist vindo do org_indices: {type(dist)}')
			#print(f'valor do dist: {dist}')
			NNdist[k] = dist[0]
		...
		#funcao para fazer dxy de um ponto baseado na posicao dele na matriz					
		...
		OII = np.sqrt(np.trapz((Besag_obs-Besag_theor)**2, x = bins)/rmax)

		return I_org, RI_org, L_org, NNCDF_theor, NNCDF_obs, Besag_theor, Besag_obs, OII
