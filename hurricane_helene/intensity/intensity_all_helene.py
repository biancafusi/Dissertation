...
def processing_data(label, data_path):
    ...
    if label == 'ERA5':
        ...
        WSPD_data = calc_wind_gf_era5(dataset_dry)
...
axs[1].set_xlabel(ax_label, fontsize=16)
...
axs[0].set_ylim(940, 1006)
for label, xpos in line_positions.items():
    axs[0].plot([xpos, xpos], [940, 1000], color='steelblue', linestyle=':', linewidth=2.0)
    axs[0].text(xpos, 1001, label, color='steelblue', fontsize=16, ha='center', va='bottom')
...
plt.tight_layout()
plt.savefig(where_to_save+"intensity_time_series_panel_FINAL.png", dpi=300)
...
# Finalizar
plt.tight_layout()
plt.savefig(where_to_save+"PANEL_intensity_errors_FINAL.png", dpi=300, bbox_inches='tight')
...
# # Salvar
# plt.tight_layout()
# plt.savefig("/mnt/beegfs/bianca.fusinato/monan/MASTERS_RESULTS/statistical_metrics/helene_results/barplot_mean_error_monan_era5_intensity_FINAL.png", dpi=300, bbox_inches='tight')